...
@injectable()
export class RealEstateServiceRepository implements IRealEstateServiceRepository {
    constructor(
        @inject(AxiosApi) private _api: AxiosApi
    ) {}

    async create(input: RealEstateServiceEntity): Promise<IResponse<RealEstateServiceEntity>> {
    ...
    async update(input: RealEstateServiceEntity): Promise<IResponse<RealEstateServiceEntity>> {
        const response = await this._api.axios({
            method: 'put',
    ...
    async delete(id: RealEstateServiceEntity): Promise<IResponse<RealEstateServiceEntity>> {
        const response = await this._api.axios({
            method: 'delete',
    ...
    async getAll(
        args: IGPaginate<Pick<RealEstateServiceEntity, 'country_id' | 'real_estate_type_id' | 'unit_price'>>
